...
function! xolox#reload#misc#buffer#is_empty() " {{{1
...
function! xolox#reload#misc#buffer#prepare(...) " {{{1
  ...
  for bufnr in tabpagebuflist()
    if xolox#reload#misc#path#equals(options['path'], bufname(bufnr))
  ...
  if !(found || xolox#reload#misc#buffer#is_empty())
    vsplit
  endif
  ...
  call xolox#reload#misc#buffer#unlock()
  silent %delete
endfunction

function! xolox#reload#misc#buffer#lock() " {{{1
  " Lock a special buffer so that its contents can no longer be edited.
  setlocal readonly nomodifiable nomodified
endfunction

function! xolox#reload#misc#buffer#unlock() " {{{1
